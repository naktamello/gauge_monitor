...
# calculate histogram to isolate the gauge from shadows etc.
def bandpass(gray_im):
    hist = cv2.calcHist([gray_im],[0],None,[256],[0,256])
    hist0=np.asarray(hist, dtype=np.int32)
    hist0=np.swapaxes(hist0,0,1)
    hist0 = smooth(np.array(hist0))

    [max, min] = peakdetect(hist0, lookahead=15, delta=20)
